...
namespace Tex2Dds
{
    class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            if (fi.Length < 0xC0)
            {
                Console.Error.WriteLine("ERROR: {0} is too small", args[0]);
                Console.ReadLine();
                return -3;
            }
            if (Path.GetExtension(args[0]).Equals(".tex"))
            {
                ...
                using (BinaryReader reader = new BinaryReader(File.OpenRead(args[0])))
                {
                    ...
                    if (internalFormat == 0)
                    {
                        Console.Error.WriteLine("ERROR: Unknown TEX format {0}.", type);
                        Console.ReadLine();
                        return -5;
                    }
            ...
            else if(Path.GetExtension(args[0]).Equals(".dds"))
            {
                string destPath = Path.GetFullPath(Path.ChangeExtension(args[0], ".tex"));
                Directory.CreateDirectory(Path.GetDirectoryName(destPath));
                ...
                using (BinaryReader reader = new BinaryReader(File.OpenRead(args[0])))
                {
                    ...
                    if (filetype == 0)
                    {
                        Console.Error.WriteLine("ERROR: Unknown DDS format {0}.", filetypecode);
                        Console.ReadLine();
                        return -5;
                    }
                    using (FileStream fsWrite = new FileStream(destPath, FileMode.Create))
            ...
            Console.WriteLine("Process finished! Thanks for using.     ——Jodo");
            return 0;
        }
